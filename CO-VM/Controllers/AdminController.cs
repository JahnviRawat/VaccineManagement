    ...
    public class AdminController : Controller
    {
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                int totalUsers = vm.Users.Count();
                int totalVaccines = vm.Vaccines.Count();
                int totalFeedbacks = vm.VaccinationFeedbacks.Count();

                // Fetch recent feedbacks (latest 10)
                var feedbacks = (from f in vm.VaccinationFeedbacks
                                 join u in vm.Users on f.UserId equals u.UserId
                                 join v in vm.Vaccines on f.VaccineId equals v.VaccineId
            ...
            {
                logger.LogError(ex, "An error occurred in Admin Index.");
